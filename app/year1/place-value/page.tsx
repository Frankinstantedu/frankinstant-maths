...
function generateQuestion() {
  ...
  return {
    ...
    tens,
    ones,
    question: `How many tens and ones are in ${number}?`,
    answer: `${tens},${ones}`,
  };
}
...
export default function PlaceValuePage() {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    // Clean up answer: support "4,5", "4 5", "4, 5"
    const cleanedInput = answer.replace(/\s+/g, "").replace(/,/g, ",");
    const correct = cleanedInput === question.answer || answer.trim() === `${question.tens}${question.ones}`;
    ...
    >
      <form onSubmit={handleSubmit} className="space-y-4">
        <input
          ...
          placeholder="e.g. 4,5 (4 tens, 5 ones)"
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-fade-in">
          ...
          >
            ...
            <p className="text-gray-700 text-lg mb-6">
              {isCorrect ? (
                "You nailed the place value!"
              ) : (
                <>
                  Not quite. <br />
                  <span className="font-bold text-gray-900">
                    {question.number} = {question.tens} tens and {question.ones} ones ({question.tens},{question.ones})
                  </span>
                </>
              )}
            </p>

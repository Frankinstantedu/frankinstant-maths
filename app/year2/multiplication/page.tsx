...
function generateQuestion() {
  // Year 2 focuses on 2, 5, and 10 times tables
  const tables = [2, 5, 10];
  const table = tables[Math.floor(Math.random() * tables.length)];
  const factor = Math.floor(Math.random() * 10) + 1; // 1 through 10
    ...
    >
      <form onSubmit={handleSubmit} className="space-y-4">
        <input
          ...
          className="border-2 border-gray-300 rounded-lg p-3 text-2xl text-center w-full focus:outline-none focus:border-emerald-500"
        ...
        <button
          ...
          className="bg-emerald-600 hover:bg-emerald-700 text-white font-bold px-6 py-3 rounded-lg w-full disabled:opacity-50 transition-colors text-lg"
        >
          Check Answer
        </button>
      </form>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-fade-in">
          <div
            className={`bg-white rounded-2xl p-6 text-center shadow-2xl max-w-sm w-full transform transition-all border-4 ${
              isCorrect ? "border-emerald-500" : "border-amber-400"
            }`}
          >
              ...
              {isCorrect ? (
                ...
                <>
                  ...
                  <span className="font-bold text-gray-900">
                    {question.factor} × {question.table} = {question.answer}
                  </span>
                </>
              )}
            </p>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-fade-in">
          <div className="bg-white rounded-3xl p-8 text-center shadow-2xl max-w-md w-full border-4 border-emerald-500">
            ...
            <div className="flex gap-4">
              <button
                onClick={restartQuiz}
                className="flex-1 bg-emerald-500 hover:bg-emerald-600 text-white font-bold py-3 rounded-xl text-lg shadow-lg shadow-emerald-200 transition-transform active:scale-95"
              >
                Play Again 🚀
              </button>
            </div>
